...
const map = (raw: any): any => ({
  ...
  status: raw.status ?? (raw.isActive === false ? 'suspended' : 'approved'), isActive: raw.isActive ?? raw.status === 'approved',
...
export const barbershopsAPI = {
  ...
  async updateBarbershop(id: string, updates: Partial<Barbershop>): Promise<Barbershop> { return map(await put<any>(`/tenants/${id}/settings`, updates)); },
